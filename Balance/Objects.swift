...
class Objects {
    var objectFrames=Array<Array<SKTexture>>()
    let OBJECT_MAX = 7
    ...
    init(imagesAtlas:SKTextureAtlas) {
        ...
        levels = ConfigurationManager.sharedInstance.levels
        for level in 0..<levels.count {
            objectFrames.append(Array<SKTexture>())
            for i in 1...levels[level].objectTypeCount {
                objectFrames[level].append(imagesAtlas.textureNamed("\(levels[level].imageName)\(i)\(deviceType).png"))
            }
        }
    }
    func getObject(objectNo:Int,levelNo:Int)->SKSpriteNode {
        var object=SKSpriteNode(texture: objectFrames[levelNo][objectNo])
        return(object)
    }
    func generatePhysics(var object:SKSpriteNode,objectNo:Int,levelNo:Int) {
        object.physicsBody = getPhysicsBody(object, objectNo: objectNo, levelNo:levelNo)
    ...
    func getPhysicsBody(sprite:SKSpriteNode, objectNo:Int, levelNo:Int)->SKPhysicsBody {
        switch levelNo {
        case 0:
            switch objectNo {
            case 0,1:
        ...
        case 1:
            ...
            case 1:
                ...
                var path = CGPathCreateMutable()
                CGPathMoveToPoint(path, nil, 86 - offsetX, 32 - offsetY);
                ...
                CGPathAddLineToPoint(path, nil, 58 - offsetX, 3 - offsetY);
                //CGPathAddLineToPoint(path, nil, 58 - offsetX, 32 - offsetY);
            ...
            case 3:
                ...
                var path = CGPathCreateMutable()
                CGPathMoveToPoint(path, nil, 58 - offsetX, 32 - offsetY);
        ...
        default:
            println("ERROR: LEVEL NOT DEFINED")
